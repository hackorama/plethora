$(function() {
	...
	function updateText(name, value){
		consoleLog("text update : " + name + " = " + value);
		elem = $(jqid(name));
		elem.text(value);
	}
	...
	function onNamesReceived(serverdata) {
		...
		if (names.length <= 0){
			$("#messages").text("Waiting for metrics to be available ... " + formatSeconds(retryIntervalSeconds*initRetry));
			initRetry++;
			setTimeout(getMetricNames, retryIntervalSeconds * 1000);
			return;	
		}
		$("#messages").text("");
		$("#metrics").show();
